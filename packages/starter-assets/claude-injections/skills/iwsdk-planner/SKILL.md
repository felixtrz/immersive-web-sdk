...
1. Derive research domains from the spec (typically: input & interaction,
   physics, locomotion, UI, audio, environment/lighting, assets/levels, and
   AR-specific surfaces if applicable).
2. Fan out one research agent per domain (or do them in sequence). Each
...
3. Merge into `design/TECH_PLAN.md`: World.create feature-flag block,
...
4. Audit against the reinvention-risk table in `references/api-reference.md`:
