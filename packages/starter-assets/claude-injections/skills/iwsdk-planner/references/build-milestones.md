...
--no-metaspatial          # ALWAYS in headless/cloud environments (the
                          # Meta Spatial Editor is a desktop GUI app)
...
```bash
npx @iwsdk/create@latest bowling-vr --yes --mode vr --physics --grabbing --no-locomotion --no-metaspatial
```

If `@latest` misbehaves, fall back to the pinned known-good version the docs
use (see iwsdk.dev — e.g. `@iwsdk/create@0.4.1`).
...
- Sub-agent brief = paths to GAME_SPEC/TECH_PLAN/ARCHITECTURE + the
  **absolute app-root path** (run all commands from there — sub-agent
  ...
  `.claude/skills/iwsdk-planner/references/api-reference.md` with the
...
## Common World.create shapes

```typescript
// VR game (stationary or room-scale)
World.create(container, {
  xr: { sessionMode: SessionMode.ImmersiveVR, offer: 'once' },
  features: {
    physics: true,
    grabbing: true,
    locomotion: false,
